...
#
#  generate multi molecule fingerprint
#  gen_mmfp.pl
#
...
#
#
# Usage: gen_mmfp.pl database_actives [database_inactives]
#
...
#
# Output: database_actives_mmfp.asc
#
# output format
#
# 166 MACCS     1st line holds number of bits in that fingerprint
# 0.00000       next lines contain frequencies (actives - decoys)
# -0.03030
# 0.22222
# ...
# 880 PubChem
...
for ($j = 1; $j <= $nfp; $j++) { # init frequencies
  for ($k = 1; $k <= $lfp[$j]; $k++) { 
     $mmfp[$j][$k] = 0; # actives
     $mmfd[$j][$k] = 0; # decoys
   }
...
#   print "$t1 ";
    ...
    $tt = $t2 / $t1; # divide by occurrence
#   print "$tt " ;
#   $mmfp[$i][$l] = $mmfp[$i][$l] / $nactives; # divide by occurrence
    $mmfp[$i][$l] = $tt ;
#   print "$mmfp[$i][$l]\n" ;
  }
}
...
for ($i = 1; $i <= $nfp; $i++) { # loop over fingerprints
  ...
  for ($k = 1; $k <= $lfp[$i]; $k++) { # loop over bits in fingerprint
    if ($ndecoys > 0) {
      $temp = $mmfp[$i][$k] - $mmfd[$i][$k] ; # difference  
    }
    else {
      $temp = $mmfp[$i][$k] ;
    }
    printf DATA ("%7.5f",$temp) ;
    print  DATA "\n" ;
  } 
}
