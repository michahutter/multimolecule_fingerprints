...
# Difference to usual AUC: here the score covers the range from -999.9 to +999.9
#
# Usage: gen_auc_bayes.pl <scores_actives> <scores_inactives>
#
...
$scmax = -999.9 ; # maximum score found
$scmin =  999.9 ; # minimum score found
...
while(defined($i = <ALIST>)) {
  ...
  if (defined($cols[2])) {   
    if (looks_like_number($cols[2])) { # is a number?
      $actf1[$nactives]=$cols[2] ;
      if ($cols[2] > $scmax) {$scmax = $cols[2]}
    }
    else {
      $nactives--;
      next ;
  ...
  if (defined($cols[3])) {   
    if (looks_like_number($cols[3])) {
      $actf2[$nactives]=$cols[3] ;
      if ($cols[3] > $scmax) {$scmax = $cols[3]}
    }
    else {
      $nactives--;
      next ;
...
while(defined($j = <DLIST>)) {
  ...
  if (defined($cols[2])) {   
    if (looks_like_number($cols[2])) {
      $inaf1[$ndecoys]=$cols[2] ;
      if ($cols[2] < $scmin) {$scmin = $cols[2]}
    }
    else {
      $ndecoys--;
      next ;
  ...
  if (defined($cols[3])) {   
    if (looks_like_number($cols[3])) {
      $inaf2[$ndecoys]=$cols[3] ;
      if ($cols[3] < $scmin) {$scmin = $cols[3]}
    }
    else {
      $ndecoys--;
      next ;
...
# determine step with for bins
$stepw = 50 ; # step width (number of bins)
$low  = $scmin ;
$high = $scmax ;
if (abs($scmin) > $scmax) { # equal amount of bins for + and - scores
  $high = abs($scmin) ;
}
else {
  $low  = $scmax * -1.0 ;
